...
overview <- overview %>%
  dplyr::select(-c(site_years_with_phenophase,
                   abundance,
                   dbh_max_mm_census,
                   mating_system,
                   diaspora,
...
overview <- overview %>%
  mutate(percentage_site_years_with_phenophase = round(ratio_site_years_with_phenophase, digits=3) *100,
         basal_area_percentage = round(basal_area_percentage, digits=2)) %>%
...
tseries <- read.csv("data/timeseries_crosscorrelations.csv",
                     header = TRUE,
                     sep = ",",
                     stringsAsFactors = FALSE)
...
#--------------------------------------------------------------------
#--------------------------------------------------------------------
four_dorm <- read.csv("data/fourier_leaf_dormancy.csv",
                    header = TRUE,
                    sep = ",",
                    stringsAsFactors = FALSE)
four_turn <- read.csv("data/fourier_leaf_turnover.csv",
                      header = TRUE,
                      sep = ",",
                      stringsAsFactors = FALSE)
...
table2 <- overview %>%
  ...
  dplyr::select(phenophase,
                ...
                nr_indiv,
                site_years,
                percentage_site_years_with_phenophase,
                total_nr_events,
                cycle_category,
                cycle_dom_months,
...
write.table(table2, "data/SI_table2_dormancy.csv",
            quote = FALSE,
            col.names = TRUE,
            row.names = FALSE,
            sep = ",")
...
write.table(table3, "data/SI_table3_turnover.csv",
            quote = FALSE,
            col.names = TRUE,
            row.names = FALSE,
            sep = ",")
...
write.table(table_ms, "data/table1_manuscript.csv",
            quote = FALSE,
            col.names = TRUE,
            row.names = FALSE,
            sep = ",")
