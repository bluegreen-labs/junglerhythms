...
#----------------------------------------------------------------------
# average year
mmonthly <- as.data.frame(tapply(climate$precip,list(climate$month),mean, na.rm = TRUE))
colnames(mmonthly)[1] <- "mmonthly"
mmonthly$month <- rownames(mmonthly)
climate <- merge(climate,mmonthly,by = "month")
# difference
climate$diff <- climate$precip - climate$mmonthly
#----------------------------------------------------------------------

# sd(climate$diff)
# climate$diff2 <- ifelse(abs(climate$diff) > sd(climate$diff)*1.96, climate$diff, 0)
...
df_dec_dorm$phase <- ifelse(df_dec_dorm$corr_dormancy_precip_timing == "0", "in-phase",
                            ifelse(df_dec_dorm$corr_dormancy_precip_timing %in% c("-1","-2","-3","-4","-5"), "lag",
                                   # ifelse(df_dec_dorm$corr_dormancy_precip_timing %in% c("1","2","3"), "lead",
                                   NA))

df_dec_dorm$relation <- ifelse(df_dec_dorm$corr_dormancy_precip > 0, "pos",
                               ifelse(df_dec_dorm$corr_dormancy_precip < 0, "neg", NA))
# tapply(df_dec_dorm$corr_dormancy_precip, list(df_dec_dorm$phase,df_dec_dorm$relation), length)
counts_dec_dorm <- length(df_dec_dorm$species_full)

df_dec_dorm <- df_dec_dorm[!(is.na(df_dec_dorm$phase)),]

df_dec_dorm$title <- "Dormancy" # for title facet_wrap

p_dec_dorm_precip <- ggplot(data = df_dec_dorm,
                          aes(x = phase,
                              group = relation,
                              fill = relation)) +
  ...
  labs(y = "% corr with precipitation",
       x = "",
       title = "") +
...
p_dec_dorm_precip <- p_dec_dorm_precip + facet_grid(. ~ title) +
...
df_dec_turn$phase <- ifelse(df_dec_turn$corr_turnover_precip_timing == "0", "in-phase",
                            ifelse(df_dec_turn$corr_turnover_precip_timing %in% c("-1","-2","-3","-4","-5"), "lag",
                                   # ifelse(df_dec_turn$corr_turnover_precip_timing %in% c("1","2","3"), "lead",
                                   NA))

df_dec_turn$relation <- ifelse(df_dec_turn$corr_turnover_precip > 0, "pos",
                               ifelse(df_dec_turn$corr_turnover_precip < 0, "neg", NA))
# tapply(df_dec_turn$corr_dormancy_precip, list(df_dec_turn$phase,df_dec_turn$relation), length)
counts_dec_turn <- length(df_dec_turn$species_full)
df_dec_turn <- df_dec_turn[!(is.na(df_dec_turn$phase)),]

df_dec_turn$title <- "Turnover" # for title facet_wrap

p_dec_turn_precip <- ggplot(data = df_dec_turn,
                          aes(x = phase,
                              group = relation,
                              fill = relation,
                              na.rm = TRUE)) +
...
p_dec_turn_precip <- p_dec_turn_precip + facet_grid(. ~ title) +
  theme(strip.background = element_rect(fill="grey85", colour = "white"),
        strip.text = element_text(size=12, colour="black", hjust = 0.5))
...
df_ever_dorm$phase <- ifelse(df_ever_dorm$corr_dormancy_precip_timing == "0", "in-phase",
                             ifelse(df_ever_dorm$corr_dormancy_precip_timing %in% c("-1","-2","-3","-4","-5"), "lag",
                                    # ifelse(df_ever_dorm$corr_dormancy_precip_timing %in% c("1","2","3"), "lead",
                                    NA))

df_ever_dorm$relation <- ifelse(df_ever_dorm$corr_dormancy_precip > 0, "pos",
                                ifelse(df_ever_dorm$corr_dormancy_precip < 0, "neg", NA))
# tapply(df_ever_dorm$corr_dormancy_precip, list(df_ever_dorm$phase,df_ever_dorm$relation), length)
counts_ever_dorm <- length(df_ever_dorm$species_full)
df_ever_dorm <- df_ever_dorm[!(is.na(df_ever_dorm$phase)),]
df_ever_dorm$title <- "Dormancy" # for title facet_wrap

p_ever_dorm_precip <- ggplot(data = df_ever_dorm,
                           aes(x = phase,
                               group = relation,
                               fill = relation)) +
...
p_ever_dorm_precip <- p_ever_dorm_precip + facet_grid(. ~ title) +
  theme(strip.background = element_rect(fill="grey85", colour = "white"),
        strip.text = element_text(size=12, colour="black", hjust = 0.5))
...
df_ever_turn$phase <- ifelse(df_ever_turn$corr_turnover_precip_timing == "0", "in-phase",
                             ifelse(df_ever_turn$corr_turnover_precip_timing %in% c("-1","-2","-3","-4","-5"), "lag",
                                    # ifelse(df_ever_turn$corr_turnover_precip_timing %in% c("1","2","3"), "lead",
                                    NA))

df_ever_turn$relation <- ifelse(df_ever_turn$corr_turnover_precip > 0, "pos",
                                ifelse(df_ever_turn$corr_turnover_precip < 0, "neg", NA))
# tapply(df_ever_turn$corr_dormancy_precip, list(df_ever_turn$phase,df_ever_turn$relation), length)
counts_ever_turn <- length(df_ever_turn$species_full)
df_ever_turn <- df_ever_turn[!(is.na(df_ever_turn$phase)),]
df_ever_turn$title <- "Turnover"

p_ever_turn_precip <- ggplot(data = df_ever_turn,
                             aes(x = phase,
                                 group = relation,
                                 fill = relation)) +
...
p_ever_turn_precip <- p_ever_turn_precip + facet_grid(. ~ title) +
  theme(strip.background = element_rect(fill="grey85", colour = "white"),
        strip.text = element_text(size=12, colour="black", hjust = 0.5))
#-------

grid.arrange(arrangeGrob(p_dec_dorm_precip, p_dec_turn_precip,top=textGrob("Deciduous",gp=gpar(fontsize=15)),ncol=2,widths = c(1,0.7)),
             arrangeGrob(p_ever_dorm_precip, p_ever_turn_precip,top=textGrob("Evergreen",gp=gpar(fontsize=15)), ncol=2),
             ncol = 2,widths = c(1,0.8))

# my_g <- grobTree(rectGrob(gp=gpar(fill="grey85", colour="white",vjust=2)),
#                  textGrob("Market Updates", x=0.5, hjust=0.5,
#                           gp=gpar(col="black", cex=1.5))

# legend <- get_legend(p_ever_turn_precip)


p_all<- grid.arrange(arrangeGrob(p_dec_dorm_precip, p_dec_turn_precip,
                         top=textGrob("Deciduous",gp=gpar(fontsize=12),hjust = 0.05, vjust = 2),
                         ncol=2,widths = c(1,0.7)),
             arrangeGrob(p_ever_dorm_precip, p_ever_turn_precip,
                         top=textGrob("Evergreen",gp=gpar(fontsize=12),vjust = 2),
                         ncol=2),
             arrangeGrob(p_dec_dorm_insol, p_dec_turn_insol,ncol=2,widths = c(1,0.7)),
             arrangeGrob(p_ever_dorm_insol, p_ever_turn_insol,ncol=2),
             arrangeGrob(p_dec_dorm_tmax, p_dec_turn_tmax,ncol=2,widths = c(1,0.7)),
             arrangeGrob(p_ever_dorm_tmax, p_ever_turn_tmax,ncol=2),
             ncol = 2,widths = c(1,0.8),
             heights = c(1,0.8,1))

pdf("~/Desktop/figure3_corr.pdf",6,6) # 5,10)
plot(p_all)
dev.off()
#----------------------------------------------------------------------------------
#----------------------------------------------------------------------------------
...
test <- df_dec_dorm %>%
  filter(corr_dormancy_precip_timing == "0")

test <- df_dec_dorm %>%
  filter(corr_dormancy_precip_timing %in% c("-1","-2","-3"))
mean(as.numeric(test$corr_dormancy_precip_timing), na.rm=TRUE)
...
test <- df_dec_turn %>%
  filter(corr_turnover_precip_timing == "0")

test <- df_dec_turn %>%
  filter(corr_turnover_precip_timing %in% c("-1","-2","-3"))
mean(as.numeric(test$corr_turnover_precip_timing), na.rm=TRUE)
...
#---------------------
# clim <- read.csv("data/yangambi_km5_monthly_kasongo.csv")
# cor.test(clim$precip, clim$sun, method = 'pearson')
# cor.test(clim$precip, clim$tmax, method = 'pearson')
# cor.test(clim$sun, clim$tmax, method = 'pearson')
#
# plot(clim$precip, clim$sun)
# plot(clim$precip, clim$tmax)
# plot(clim$sun, clim$tmax)
