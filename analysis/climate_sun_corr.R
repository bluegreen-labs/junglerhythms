...
#   filter(year >= 1951,
#          year < 2012)
# # stationarity and autocorr sun
# adf.test(climate$sun) # p value lower then 0.05 = stationary
# kpss.test(climate$sun) # p value higher then 0.05 = stationary
# # Check for autocorrelation in a timeseries
# autocorr <- acf(climate$sun)

climate.avg <- read.csv("data/ClimData_monthly_avg.csv",header = TRUE,sep = ",")
climate.avg = climate.avg[,(names(climate.avg) %in% c("Month","insol_JR"))]
...
p_climate <- ggplot() +
  ...
  scale_x_date(date_breaks = "1 year",
               date_labels = "%Y",
               limits = as.Date(c('1931-01-01','1932-01-01'))) +
  theme_minimal() +
...
for(i in 1:length(species_list)){
  ...
  output_dormancy[i,2:14] <- as.numeric(corr.insol_JR$acf)
...
output_dormancy$corr <- ifelse(abs(output_dormancy[,8]) > output_dormancy$ci, output_dormancy[,8], # 0
                               ...
                               # ifelse(abs(output_dormancy[,9]) > output_dormancy$ci, output_dormancy[,9], # t+1
...
output_dormancy$corr.timing <- ifelse(abs(output_dormancy[,8]) > output_dormancy$ci, "0", # 0
                                      ...
                                      # ifelse(abs(output_dormancy[,9]) > output_dormancy$ci, "1", # t+1
...
dormancy_insol_JR <- output_dormancy[,(names(output_dormancy) %in% c("species_full",
                                                                   "corr",
                                                                   "corr.timing"))]
...
for(i in 1:length(species_list)){
  ...
  output_turnover[i,2:14] <- as.numeric(corr.insol_JR$acf)
                                             ...
                                             ifelse(abs(output_turnover[,5]) > output_turnover$ci, output_turnover[,5], # t-3
                                                    ifelse(abs(output_turnover[,4]) > output_turnover$ci, output_turnover[,4], # t-4
                                                           ifelse(abs(output_turnover[,3]) > output_turnover$ci, output_turnover[,3], # t-5
                                                                  # ifelse(abs(output_turnover[,9]) > output_turnover$ci, output_turnover[,9], # t+1
                                                    ...
                                                    ifelse(abs(output_turnover[,5]) > output_turnover$ci, "-3", # t-3
                                                           ifelse(abs(output_turnover[,4]) > output_turnover$ci, "-4", # t-4
                                                                  ifelse(abs(output_turnover[,3]) > output_turnover$ci, "-5", # t-5
                                                                         # ifelse(abs(output_turnover[,9]) > output_turnover$ci, "1", # t+1
...
turnover_insol_JR <- output_turnover[,(names(output_turnover) %in% c("species_full",
                                                                     "corr",
                                                                     "corr.timing"))]
...
output <- merge(overview[1], dormancy_insol_JR, by = "species_full", all.x = TRUE)
output <- merge(output, turnover_insol_JR, by = "species_full", all.x = TRUE)

write.table(output, "data/timeseries_sunhours_correlations.csv",
            quote = FALSE,
            col.names = TRUE,
            row.names = FALSE,
            sep = ",")
...
df_dec_dorm$phase <- ifelse(df_dec_dorm$corr_dormancy_insol_JR_timing == "0", "in-phase",
                            ifelse(df_dec_dorm$corr_dormancy_insol_JR_timing %in% c("-1","-2","-3","-4","-5"), "lag",
                                   # ifelse(df_dec_dorm$corr_dormancy_insol_JR_timing %in% c("1","2","3"), "lead",
                                   NA))

df_dec_dorm$relation <- ifelse(df_dec_dorm$corr_dormancy_insol_JR > 0, "pos",
                               ifelse(df_dec_dorm$corr_dormancy_insol_JR < 0, "neg", NA))

# df_dec_dorm$corr_dormancy_insol_JR_timing <- as.numeric(df_dec_dorm$corr_dormancy_insol_JR_timing)
# tapply(df_dec_dorm$corr_dormancy_insol_JR_timing, list(df_dec_dorm$phase,df_dec_dorm$relation), mean, na.rm=TRUE)
# tapply(df_dec_dorm$corr_dormancy_insol_JR_timing, list(df_dec_dorm$phase,df_dec_dorm$relation), sd, na.rm=TRUE)
...
p_dec_dorm_insol <- ggplot(data = df_dec_dorm,
                          aes(x = phase,
                              group = relation,
                              fill = relation)) +
...
df_dec_turn$phase <- ifelse(df_dec_turn$corr_turnover_insol_JR_timing == "0", "in-phase",
                            ifelse(df_dec_turn$corr_turnover_insol_JR_timing %in% c("-1","-2","-3","-4","-5"), "lag",
                                   # ifelse(df_dec_turn$corr_turnover_insol_JR_timing %in% c("1","2","3"), "lead",
                                   NA))

df_dec_turn$relation <- ifelse(df_dec_turn$corr_turnover_insol_JR > 0, "pos",
                               ifelse(df_dec_turn$corr_turnover_insol_JR < 0, "neg", NA))
# tapply(df_dec_turn$corr_dormancy_insol_JR, list(df_dec_turn$phase,df_dec_turn$relation), length)
counts_dec_turn <- length(df_dec_turn$species_full)
df_dec_turn <- df_dec_turn[!(is.na(df_dec_turn$phase)),]

p_dec_turn_insol <- ggplot(data = df_dec_turn,
                          aes(x = phase,
                              group = relation,
                              fill = relation,
                              na.rm = TRUE)) +
...
df_ever_dorm$phase <- ifelse(df_ever_dorm$corr_dormancy_insol_JR_timing == "0", "in-phase",
                             ifelse(df_ever_dorm$corr_dormancy_insol_JR_timing %in% c("-1","-2","-3","-4","-5"), "lag",
                                    # ifelse(df_ever_dorm$corr_dormancy_insol_JR_timing %in% c("1","2","3"), "lead",
                                    NA))

df_ever_dorm$relation <- ifelse(df_ever_dorm$corr_dormancy_insol_JR > 0, "pos",
                                ifelse(df_ever_dorm$corr_dormancy_insol_JR < 0, "neg", NA))

# tapply(df_ever_dorm$corr_dormancy_insol_JR, list(df_ever_dorm$phase,df_ever_dorm$relation), length)
# df_ever_dorm$corr_dormancy_insol_JR_timing <- as.numeric(df_ever_dorm$corr_dormancy_insol_JR_timing)
# tapply(df_ever_dorm$corr_dormancy_insol_JR_timing, list(df_ever_dorm$phase,df_ever_dorm$relation), mean, na.rm=TRUE)
# tapply(df_ever_dorm$corr_dormancy_insol_JR_timing, list(df_ever_dorm$phase,df_ever_dorm$relation), sd, na.rm=TRUE)


counts_ever_dorm <- length(df_ever_dorm$species_full)
df_ever_dorm <- df_ever_dorm[!(is.na(df_ever_dorm$phase)),]

p_ever_dorm_insol <- ggplot(data = df_ever_dorm,
                           aes(x = phase,
                               group = relation,
                               fill = relation)) +
...
df_ever_turn$phase <- ifelse(df_ever_turn$corr_turnover_insol_JR_timing == "0", "in-phase",
                             ifelse(df_ever_turn$corr_turnover_insol_JR_timing %in% c("-1","-2","-3","-4","-5"), "lag",
                                    # ifelse(df_ever_turn$corr_turnover_insol_JR_timing %in% c("1","2","3"), "lead",
                                    NA))

df_ever_turn$relation <- ifelse(df_ever_turn$corr_turnover_insol_JR > 0, "pos",
                                ifelse(df_ever_turn$corr_turnover_insol_JR < 0, "neg", NA))
# tapply(df_ever_turn$corr_dormancy_insol_JR, list(df_ever_turn$phase,df_ever_turn$relation), length)
# df_ever_turn$corr_dormancy_insol_JR_timing <- as.numeric(df_ever_turn$corr_dormancy_insol_JR_timing)
# tapply(df_ever_turn$corr_dormancy_insol_JR_timing, list(df_ever_turn$phase,df_ever_turn$relation), mean, na.rm=TRUE)
# tapply(df_ever_turn$corr_dormancy_insol_JR_timing, list(df_ever_turn$phase,df_ever_turn$relation), sd, na.rm=TRUE)

counts_ever_turn <- length(df_ever_turn$species_full)
df_ever_turn <- df_ever_turn[!(is.na(df_ever_turn$phase)),]

p_ever_turn_insol <- ggplot(data = df_ever_turn,
                           aes(x = phase,
                               group = relation,
                               fill = relation)) +
...
grid.arrange(arrangeGrob(p_dec_dorm_insol, p_dec_turn_insol,top=textGrob("Deciduous",gp=gpar(fontsize=15)),ncol=2,widths = c(1,0.8)),
             arrangeGrob(p_ever_dorm_insol, p_ever_turn_insol,top=textGrob("Evergreen",gp=gpar(fontsize=15)), ncol=2),
...
test <- df_dec_dorm %>%
  filter(corr_dormancy_insol_JR_timing == "0")

test <- df_dec_dorm %>%
  filter(corr_dormancy_insol_JR_timing %in% c("-1","-2","-3","-4","-5"))
mean(as.numeric(test$corr_dormancy_insol_JR_timing), na.rm=TRUE)
...
test <- df_dec_turn %>%
  filter(corr_turnover_insol_JR_timing == "0")

test <- df_dec_turn %>%
  filter(corr_turnover_insol_JR_timing %in% c("-1","-2","-3","-4","-5"))
mean(as.numeric(test$corr_turnover_insol_JR_timing), na.rm=TRUE)
