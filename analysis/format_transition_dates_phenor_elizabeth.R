...
transition_dates <- data %>%
  ...
  group_by(id, year, species_full) %>%
  arrange(date) %>%  # sort by sort, earliest date first
  ...
  mutate(diff_value = ifelse(c(value - lag(value)) == 1,
                             1, 0)) %>%
  dplyr::summarise(week = week[which(diff_value == 1)[1]]) %>%
  na.omit() %>%
  ungroup()
...
bla <- transition_dates %>%
  ...
  # filter(length(week) > 2) %>%    # only species with more than 10 events
  summarise(
...
bla <- bla %>%
  arrange(BA) %>%
  mutate(y_value = (length(species_full)))
